///
/// * based on EDAnalyzer written by S. Harper:
///   - https://github.com/Sam-Harper/usercode/blob/3aeac53fc958eb5343a2f41fffd556b56e00c1c9/TrigTools/plugins/Ele32DoubleL1ToSingleL1Example.cc
///
/// * converted to EDProducer in order to create a bool
///
...
// note the use of anonymous namespace to avoid linking conflicts
namespace {
...
class Ele32DoubleL1ToSingleL1FlagProducer : public edm::EDProducer
{
 private:
 ...
 public:
  ...
  explicit Ele32DoubleL1ToSingleL1FlagProducer(const edm::ParameterSet& iPara);
  ~Ele32DoubleL1ToSingleL1FlagProducer() {}
...
Ele32DoubleL1ToSingleL1FlagProducer::Ele32DoubleL1ToSingleL1FlagProducer(const edm::ParameterSet& iPara)
  : trigObjsTag_(iPara.getParameter<edm::InputTag>("triggerObjects"))
  , trigResultsTag_(iPara.getParameter<edm::InputTag>("triggerResults"))
  ...
  , elesToken_(consumes<edm::View<pat::Electron> >(iPara.getParameter<edm::InputTag>("electrons")))
{
  produces<bool>();
}

void Ele32DoubleL1ToSingleL1FlagProducer::beginJob()
{
}

void Ele32DoubleL1ToSingleL1FlagProducer::produce(edm::Event& iEvent,const edm::EventSetup& iSetup)
{
  ...
  if(trigResultsHandle.isValid() && trigObjsHandle.isValid() && elesHandle.isValid())
  {
    // the filter names are all packed in miniAOD so we need to create a new collection of them which are unpacked
...
//  	std::cout <<" ele "<<ele.et()<<" "<<eta<<" "<<phi<<" passes HLT_Ele32_WPTight_Gsf"<<std::endl;
  ...
  auto pass_ptr = std::make_unique<bool>(pass);
  iEvent.put(std::move(pass_ptr));

  return;
}

void Ele32DoubleL1ToSingleL1FlagProducer::fillDescriptions(edm::ConfigurationDescriptions& descriptions)
{
  edm::ParameterSetDescription desc;
  desc.add<edm::InputTag>("triggerObjects", edm::InputTag("triggerObjects"))->setComment("std::vector<pat::TriggerObjectStandAlone>");
  desc.add<edm::InputTag>("triggerResults", edm::InputTag("triggerResults"))->setComment("edm::triggerResults");
  desc.add<edm::InputTag>("electrons", edm::InputTag("electrons"))->setComment("edm::View<pat::Electron>");
  descriptions.add("ele32DoubleL1ToSingleL1FlagProducer", desc);
}

DEFINE_FWK_MODULE(Ele32DoubleL1ToSingleL1FlagProducer);

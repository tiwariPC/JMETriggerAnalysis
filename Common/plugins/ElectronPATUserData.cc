...
ElectronPATUserData::ElectronPATUserData(const edm::ParameterSet& iConfig)
  ...
  {
    userInt_stringSelects_.emplace_back(std::pair<std::string, StringCutObjectSelector<pat::Electron, true> >(vname, pset_userInt_stringSelects.getParameter<std::string>(vname)));
  }
  // -----------------
  ...
  {
    userFloat_stringFuncs_.emplace_back(std::pair<std::string, StringObjectFunction<pat::Electron, true> >(vname, pset_userFloat_stringFuncs.getParameter<std::string>(vname)));
  }
  // -----------------
...
void ElectronPATUserData::produce(edm::Event& iEvent, const edm::EventSetup& iSetup)
{
  ...
  for(unsigned int i_ele=0; i_ele<patElecs->size(); ++i_ele){
    ...
    for(unsigned int i=0; i<v_vmap_bool.size(); ++i)
    {
      ...
      {
        throw cms::Exception("InputError")
          << "@@@ ElectronPATUserData::produce -- object reference not found in ValueMap<bool> \"" << vmaps_bool_.at(i) << "\"";
      }
    }
    // -----------------
    ...
    for(unsigned int i=0; i<v_vmap_float.size(); ++i)
    {
      ...
      {
        throw cms::Exception("InputError")
          << "@@@ ElectronPATUserData::produce -- object reference not found in ValueMap<float> \"" << vmaps_float_.at(i) << "\"";
      }
    }
    // -----------------
    ...
    for(const auto& userfloat_copycat : v_float_copycats_)
    {
      ...
      {
        throw cms::Exception("InputError") << "@@@ ElectronPATUserData::produce -- PAT user-float key \""+ref+"\" not found";
      }

      if(ele.hasUserFloat(out) == true)
      {
        throw cms::Exception("InputError") << "@@@ ElectronPATUserData::produce -- target PAT user-float key \""+out+"\" already exists";
      }
    ...
    const float SIP3D = ((ele.edB(pat::Electron::PV3D) != 0.) ? (ele.dB(pat::Electron::PV3D) / ele.edB(pat::Electron::PV3D)) : +9999.);
    ...
    for(const auto& i_strNfunc : userInt_stringSelects_)
    {
      ...
      {
        throw cms::Exception("InputError") << "@@@ ElectronPATUserData::produce -- PAT user-int label already exists: " << i_str;
      }
    ...
    for(const auto& i_strNfunc : userFloat_stringFuncs_)
    {
      ...
      {
        throw cms::Exception("InputError") << "@@@ ElectronPATUserData::produce -- PAT user-float label already exists: " << i_str;
      }

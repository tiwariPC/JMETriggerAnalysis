  ...
  , maxNVertices_(iConfig.getParameter<int>("maxNVertices"))
{
  if(minSumPt2FractionWrtMax_ >= 1.){
    ...
    throw cms::Exception("Input") << "invalid (>= 1.0) value for configuration parameter \"minSumPt2FractionWrtMax\": " << minSumPt2FractionWrtMax_;
  }
...
void PixelVerticesSelector::produce(edm::Event& iEvent, const edm::EventSetup& iSetup){
  ...
  std::sort(vec_indexAndFOM.begin(), vec_indexAndFOM.end(), [](const PixelVerticesSelector::indexAndFOM& foo1, const PixelVerticesSelector::indexAndFOM& foo2){return (foo1.fom > foo2.fom);});

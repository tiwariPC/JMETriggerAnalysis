...
void TracksClosestToFirstVerticesSelector::produce(edm::Event& iEvent, const edm::EventSetup& iSetup) {
  ...
  if(vertices->size() > 0){
    ...
    for(const auto& track : *tracks){
      ...
      LogDebug("Output") << "track-vertex association: track.vz=" << track.vz()
        << ", min(abs(track.vz - vertex.z))=" << trkVtx_minDz << ", vertex-index=" << trkVtx_minDz_vtxIdx
  ...
  LogDebug("Output") << "output collection contains " << out_tracks->size() << " tracks (input: " << tracks->size() << " tracks)";
...
void TracksClosestToFirstVerticesSelector::fillDescriptions(edm::ConfigurationDescriptions& descriptions){
  ...
  desc.add<edm::InputTag>("vertices", edm::InputTag(""))->setComment("input collection of vertices (if empty, no tracks will be retained)");
  desc.add<int>("maxNVertices", -1)->setComment("retain only tracks associated to one of the first N vertices (if negative, all vertices are considered)");
  desc.add<double>("maxDeltaZ", -1.)->setComment("maximum (absolute) delta-Z distance between track.vz and vertex.z for track-vertex matching (if negative, criterion is not applied");

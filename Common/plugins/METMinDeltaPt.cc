...
bool METMinDeltaPt::filter(edm::Event& iEvent, const edm::EventSetup& iSetup){
  ...
  if(offlineCorrectionLevel_ == "Raw"){ offlineCorrLevel = pat::MET::Raw; }
  else if(offlineCorrectionLevel_ == "Type1"){ offlineCorrLevel = pat::MET::Type1; }
  else if(offlineCorrectionLevel_ == "Type1XY"){ offlineCorrLevel = pat::MET::Type1XY; }
  else if(offlineCorrectionLevel_ == "TypeXY"){ offlineCorrLevel = pat::MET::TypeXY; }
  else if(offlineCorrectionLevel_ == "RawCalo"){ offlineCorrLevel = pat::MET::RawCalo; }
  else if(offlineCorrectionLevel_ == "RawChs"){ offlineCorrLevel = pat::MET::RawChs; }
  else if(offlineCorrectionLevel_ == "RawTrk"){ offlineCorrLevel = pat::MET::RawTrk; }
  else {
  ...
  LogDebug("Output") << "online-MET pT=" << online_pt << ", offline-MET pT=" << offline_pt
    << ", filter condition = (" << std::abs(online_pt - offline_pt) << " > " << (minRelDiff_ * offline_pt) << ")";
...
void METMinDeltaPt::fillDescriptions(edm::ConfigurationDescriptions& descriptions){
  ...
  desc.add<std::string>("offlineCorrectionLevel")->setComment("String to select correction level of offline-MET (see enum pat::MET::METCorrectionLevel)");
  desc.add<double>("minRelDiff")->setComment("Minimum relative difference between online-pT and offline-pT (relative to offline-pT)");
